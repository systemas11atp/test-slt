	...
	<script  type="text/javascript" charset="utf-8" async defer>
		...
		function recorrele(elarreglo) {
			for (var i = 0; i < elarreglo.length; i++) {
				if(prev_handle != elarreglo[i]["Handle"]){
					prev_handle = elarreglo[i]["Handle"]
					if(cuentele >= 0 && src.length > 0 && sku.length > 0){
						arregloFinal[cuentele] = []
						arregloFinal[cuentele].push(sku)
						arregloFinal[cuentele].push(src)
						arregloFinal[cuentele].push(position)
						arregloFinal[cuentele].push(alt)
						cuentele++
						elJson = {"orden":cuentele, "sku" : sku, "src" : src,"position" : position,"alt" : alt}
						data = JSON.stringify(elJson)
						url = "https://silhouettemexico.com.mx/Automatizacion/deShopify/guardaImagenes.php"
						var recargaCarrito = {
							"async": true,
							"crossDomain": true,
							"url": url,
				...
				if(!(typeof elarreglo[i]["Variant SKU"]  === 'undefined')){
					sku.push(elarreglo[i]["Variant SKU"])
				}
				if(!(typeof elarreglo[i]["Image Src"]  === 'undefined')){
					el_link = elarreglo[i]["Image Src"].replace(/https:/g,'').replace(/\/\/cdn/g,'cdn')
					src.push(el_link)
				}
				if(!(typeof elarreglo[i]["Image Position"]  === 'undefined')){
					position.push(elarreglo[i]["Image Position"])
				}
				if(!(typeof elarreglo[i]["Image Alt Text"]  === 'undefined') && elarreglo[i]["Image Alt Text"] != "no cuenta"){
					alt.push(elarreglo[i]["Image Alt Text"])
				}else{
					alt.push("")
				}
			}
