...
class OrderCore extends ObjectModel
{
    ...
    {
        capuraLogs::nuevo_log("Order 2.1");
        if (!$this->hasInvoice()) {
            capuraLogs::nuevo_log("Order 2.2");
            if ($id = (int) $this->getOrderInvoiceIdIfHasDelivery()) {
                capuraLogs::nuevo_log("Order 2.3");
                $order_invoice = new OrderInvoice($id);
            } else {
                capuraLogs::nuevo_log("Order 2.4");
                $order_invoice = new OrderInvoice();
            }
            $order_invoice->id_order = $this->id;
            capuraLogs::nuevo_log("Order 2.5");
            if (!$id) {
                capuraLogs::nuevo_log("Order 2.6");
                $order_invoice->number = 0;
            }

            // Save Order invoice

            capuraLogs::nuevo_log("Order 2.7");
            $this->setInvoiceDetails($order_invoice);

            capuraLogs::nuevo_log("Order 2.8");
            if (Configuration::get('PS_INVOICE')) {
                capuraLogs::nuevo_log("Order 2.9");
                $this->setLastInvoiceNumber($order_invoice->id, $this->id_shop);
            }

            // Update order_carrier
            capuraLogs::nuevo_log("Order 2.10");
            ...
            if ($id_order_carrier) {
                capuraLogs::nuevo_log("Order 2.11");
            ...
            if ($use_existing_payment) {
                capuraLogs::nuevo_log("Order 2.12");
                ...
                if (count($id_order_payments)) {
                    capuraLogs::nuevo_log("Order 2.13");
            ...
            capuraLogs::nuevo_log("Order 2.14");
            Db::getInstance()->execute('
            ...
            if (Configuration::get('PS_INVOICE')) {
                capuraLogs::nuevo_log("Order 2.15");
                ...
                if (is_numeric($invoice_number)) {
                    capuraLogs::nuevo_log("Order 2.16");
                    $this->invoice_number = (int) $invoice_number;
                } else {
                    $this->invoice_number = $this->getInvoiceNumber($order_invoice->id);
                    capuraLogs::nuevo_log("Order 2.17");
                }
            }

            capuraLogs::nuevo_log("Order 2.18");
            $this->update();
        }
    }
