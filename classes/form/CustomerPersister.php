...
class CustomerPersisterCore
{
    ...
    public function save(Customer $customer, $clearTextPassword, $newPassword = '', $passwordRequired = true)
    {
        ...
        capuraLogs::nuevo_log(" CustomerPersister 1");
    ...
    {
        capuraLogs::nuevo_log(" CustomerPersister 2");
        ...
        if (Customer::customerExists($customer->email, false, true)) {
            capuraLogs::nuevo_log(" CustomerPersister 3");
            $this->errors['email'][] = $this->translator->trans(
                'An account was already registered with this email address',
                [],
                'Shop.Notifications.Error'
        ...
        $ok = $customer->save();
        capuraLogs::nuevo_log(" CustomerPersister 4");
        if ($ok) {
            capuraLogs::nuevo_log(" CustomerPersister 5");
            $this->context->updateCustomer($customer);
            $this->context->cart->update();
            $this->sendConfirmationMail($customer);
    ...
    private function sendConfirmationMail(Customer $customer)
    {
        ...
        capuraLogs::nuevo_log("CustomerPersister this->context->language->id : {$this->context->language->id}");
        return Mail::Send(
