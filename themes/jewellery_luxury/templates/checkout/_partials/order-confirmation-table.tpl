 ...
 *}
 <div id="order-items" class="col-md-12">
  ...
  <div class="order-confirmation-table">
    ...
    {$elFondo = 'local_light_bg'}
    {foreach from=$products item=product}
    <div class="order-line row {$elFondo}">
      {if $elFondo eq 'local_light_bg'}
      {$elFondo = 'local_dark_bg'}
      {else}
      {$elFondo = 'local_light_bg'}
      {/if}
          <!--
      ...
      <table>
        ...
        {/if}
        {/if}
        <tr class="font-weight-bold {$elFondo}">
          <td><span class="text-uppercase">{$totals.total.label}</span> {$labels.tax_short}</td>
          <td id="order-confirmation-table-totalConIva" style="text-align: right;"> </td>
          <script>
            sumarTotal('{$totals.total.value}','{$subtotals.tax.value}','order-confirmation-table-totalConIva')
          </script>
        </tr>
      </table>
      {/block}
